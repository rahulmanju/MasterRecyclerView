...
import android.improvised.widget.helper.ItemTouchHelper;
...
/**
 * A {@link MasterRecyclerView.LayoutManager} implementation which provides
 * similar functionality to {@link android.widget.ListView}.
 */
public class LinearLayoutManager extends MasterRecyclerView.LayoutManager implements
     ...
     * {@link #onLayoutChildren(MasterRecyclerView.Recycler, MasterRecyclerView.State)} is run.
     */
     ...
     */
    int mPendingScrollPosition = MasterRecyclerView.NO_POSITION;
    ...
    * */
     ...
     */
    @Override
    public MasterRecyclerView.LayoutParams generateDefaultLayoutParams() {
        return new MasterRecyclerView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
     ...
     * RecyclerView.
     * <p>
     * If you are using a {@link MasterRecyclerView.RecycledViewPool}, it might be a good idea to set
    ...
    @Override
    public void onDetachedFromWindow(MasterRecyclerView view, MasterRecyclerView.Recycler recycler) {
     ...
     * When set to true, If {@link MasterRecyclerView} is LTR, than it will
     * layout from RTL, if {@link MasterRecyclerView}} is RTL, it will layout
     * from LTR.
     *
     ...
     */
    protected int getExtraLayoutSpace(MasterRecyclerView.State state) {
    ...
    @Override
    public void smoothScrollToPosition(MasterRecyclerView masterRecyclerView, MasterRecyclerView.State state,
                                       int position) {
        LinearSmoothScroller linearSmoothScroller =
                new LinearSmoothScroller(masterRecyclerView.getContext()) {
                    @Override
     ...
     */
    @Override
    public void onLayoutChildren(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state) {
        // layout algorithm:
        ...
        if (mPendingSavedState != null || mPendingScrollPosition != MasterRecyclerView.NO_POSITION) {
        ...
        if (!mAnchorInfo.mValid || mPendingScrollPosition != MasterRecyclerView.NO_POSITION ||
        ...
        if (state.isPreLayout() && mPendingScrollPosition != MasterRecyclerView.NO_POSITION &&
    ...
    @Override
    public void onLayoutCompleted(MasterRecyclerView.State state) {
        super.onLayoutCompleted(state);
        mPendingSavedState = null; // we don't need this anymore
        mPendingScrollPosition = MasterRecyclerView.NO_POSITION;
        mPendingScrollPositionOffset = INVALID_OFFSET;
        mAnchorInfo.reset();
    }
     ...
     *                                 indices.
     */
    void onAnchorReady(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state,
     ...
     */
    private void layoutForPredictiveAnimations(MasterRecyclerView.Recycler recycler,
                                               MasterRecyclerView.State state, int startOffset, int endOffset) {
        ...
        final List<MasterRecyclerView.ViewHolder> scrapList = recycler.getScrapList();
        final int scrapSize = scrapList.size();
        final int firstChildPos = getPosition(getChildAt(0));
        for (int i = 0; i < scrapSize; i++) {
            MasterRecyclerView.ViewHolder scrap = scrapList.get(i);
            if (scrap.isRemoved()) {
                continue;
            }
    ...
    private void updateAnchorInfoForLayout(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state,
     ...
     */
    private boolean updateAnchorFromChildren(MasterRecyclerView.Recycler recycler,
                                             MasterRecyclerView.State state, AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
     ...
     */
    private boolean updateAnchorFromPendingData(MasterRecyclerView.State state, AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == MasterRecyclerView.NO_POSITION) {
            return false;
        }
     ...
     */
    private int fixLayoutEndGap(int endOffset, MasterRecyclerView.Recycler recycler,
                                MasterRecyclerView.State state, boolean canOffsetChildren) {
     ...
     * @return The final offset amount for children
     */
    private int fixLayoutStartGap(int startOffset, MasterRecyclerView.Recycler recycler,
                                  MasterRecyclerView.State state, boolean canOffsetChildren) {
     ...
     */
    @Override
    public int scrollHorizontallyBy(int dx, MasterRecyclerView.Recycler recycler,
     ...
     */
    @Override
    public int scrollVerticallyBy(int dy, MasterRecyclerView.Recycler recycler,
                                  MasterRecyclerView.State state) {
        if (mOrientation == HORIZONTAL) {
            return 0;
        }
    ...
    @Override
    public int computeHorizontalScrollOffset(MasterRecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeVerticalScrollOffset(MasterRecyclerView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeHorizontalScrollExtent(MasterRecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeVerticalScrollExtent(MasterRecyclerView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeHorizontalScrollRange(MasterRecyclerView.State state) {
        return computeScrollRange(state);
    }

    @Override
    public int computeVerticalScrollRange(MasterRecyclerView.State state) {
        return computeScrollRange(state);
    }

    private int computeScrollOffset(MasterRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollExtent(MasterRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollRange(MasterRecyclerView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
                                   boolean canUseExistingSpace, MasterRecyclerView.State state) {
    ...
    int scrollBy(int dy, MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state) {
     ...
     */
    private void recycleChildren(MasterRecyclerView.Recycler recycler, int startIndex, int endIndex) {
     ...
     *
     * @param recycler Recycler instance of {@link MasterRecyclerView}
     * @param dt       This can be used to add additional padding to the visible area. This is used
     *                 to detect children that will go out of bounds after scrolling, without
     *                 actually moving them.
     */
    private void recycleViewsFromStart(MasterRecyclerView.Recycler recycler, int dt) {
        if (dt < 0) {
            if (DEBUG) {
     ...
     *                 actually moving them.
     */
    private void recycleViewsFromEnd(MasterRecyclerView.Recycler recycler, int dt) {
        final int childCount = getChildCount();
        if (dt < 0) {
            if (DEBUG) {
     ...
     * @see #recycleViewsFromStart(MasterRecyclerView.Recycler, int)
     * @see #recycleViewsFromEnd(MasterRecyclerView.Recycler, int)
     * @see LinearLayoutManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(MasterRecyclerView.Recycler recycler, LayoutState layoutState) {
     ...
     */
    int fill(MasterRecyclerView.Recycler recycler, LayoutState layoutState,
             MasterRecyclerView.State state, boolean stopOnFocusable) {
    ...
    void layoutChunk(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state,
        ...
        MasterRecyclerView.LayoutParams params = (MasterRecyclerView.LayoutParams) view.getLayoutParams();
     ...
     */
    private View findReferenceChildClosestToEnd(MasterRecyclerView.Recycler recycler,
                                                MasterRecyclerView.State state) {
     ...
     * @return A View that can be used an an anchor View.
     */
    private View findReferenceChildClosestToStart(MasterRecyclerView.Recycler recycler,
                                                  MasterRecyclerView.State state) {
        return mShouldReverseLayout ? findLastReferenceChild(recycler, state) :
                findFirstReferenceChild(recycler, state);
    }

    private View findFirstReferenceChild(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state) {
        return findReferenceChild(recycler, state, 0, getChildCount(), state.getItemCount());
    }

    private View findLastReferenceChild(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state) {
        return findReferenceChild(recycler, state, getChildCount() - 1, -1, state.getItemCount());
    }

    // overridden by GridLayoutManager
    View findReferenceChild(MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state,
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((MasterRecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
     ...
     *
     * @return The adapter position of the first visible item or {@link MasterRecyclerView#NO_POSITION} if
    ...
    public int findFirstVisibleItemPosition() {
        ...
        return child == null ? MasterRecyclerView.NO_POSITION : getPosition(child);
    }
     ...
     * {@link MasterRecyclerView#NO_POSITION} if there aren't any visible items.
     ...
     * are ignored in this method.
     *
     * @return The adapter position of the last visible view or {@link MasterRecyclerView#NO_POSITION} if
     * there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
                                    MasterRecyclerView.Recycler recycler, MasterRecyclerView.State state) {
        resolveShouldLayoutReverse();
        if (getChildCount() == 0) {
            return null;
        }
    ...
    static class LayoutState {
        ...
        /**
         * Equal to {@link MasterRecyclerView.State#isPreLayout()}. When consuming scrap, if this value
        ...
        /**
         * The most recent {@link #scrollBy(int, MasterRecyclerView.Recycler, MasterRecyclerView.State)}
         * amount.
         */
         ...
         */
        List<MasterRecyclerView.ViewHolder> mScrapList = null;
         ...
         */
        boolean hasMore(MasterRecyclerView.State state) {
         ...
         */
        View next(MasterRecyclerView.Recycler recycler) {
            if (mScrapList != null) {
        ...
        private View nextViewFromScrapList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final MasterRecyclerView.LayoutParams lp = (MasterRecyclerView.LayoutParams) view.getLayoutParams();
                if (lp.isItemRemoved()) {
                    continue;
                }
        ...
        public void assignPositionFromScrapList(View ignore) {
            ...
            if (closest == null) {
                mCurrentPosition = MasterRecyclerView.NO_POSITION;
            } else {
                mCurrentPosition = ((MasterRecyclerView.LayoutParams) closest.getLayoutParams())
    ...
    public static class SavedState implements Parcelable {
        ...
        void invalidateAnchor() {
            mAnchorPosition = MasterRecyclerView.NO_POSITION;
        }
    ...
    class AnchorInfo {
        ...
        void reset() {
            mPosition = MasterRecyclerView.NO_POSITION;
        ...
        private boolean isViewValidAsAnchor(View child, MasterRecyclerView.State state) {
            MasterRecyclerView.LayoutParams lp = (MasterRecyclerView.LayoutParams) child.getLayoutParams();
